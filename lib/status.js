...
StatusController.prototype.getInfo = function (callback) {
  this.node.services.helpd.getInfo(function (err, result) {
    if (err) {
      return callback(err);
    }
    var info = {
...
StatusController.prototype.getLastBlockHash = function () {
  var hash = this.node.services.helpd.tiphash;
  return {
    syncTipHash: hash,
    lastblockhash: hash
  };
...
StatusController.prototype.getBestBlockHash = function (callback) {
  this.node.services.helpd.getBestBlockHash(function (err, hash) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.getDifficulty = function (callback) {
  this.node.services.helpd.getInfo(function (err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.sync = function (req, res) {
  ...
  this.node.services.helpd.isSynced(function (err, synced) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    if (synced) {
    ...
    self.node.services.helpd.syncPercentage(function (err, percentage) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      var info = {
        status: status,
        blockChainHeight: self.node.services.helpd.height,
        syncPercentage: Math.round(percentage),
        height: self.node.services.helpd.height,
        error: null,
        type: 'bitcore node'
      };
...
StatusController.prototype.supply = function (req, res) {
  this.getInfo(function (err, result) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
      max: 23000000,
      circulating: getCirculatingSupply(result.blocks)
    });
  });
};

function getCirculatingSupply(currentBlockHeight) {
  let circulatingSupply = 0;

  for (let i = 0; i <= currentBlockHeight; i++) {
    let currentBlockReward = 0;

    if (i === 0) {
      currentBlockReward = 0;
    } else if (i === 2) {
      currentBlockReward = 15500000;
    } else if (i < 2000) {
      // Early ages...
      // 1111/((x+1)^2)
      ...
      // CPU mining era
      // 11111/(((x+51)/6)^2)
      currentBlockReward = 3;
    } else {
      // GPU/ASIC mining era
      // 2222222/(((x+2600)/9)^2)
      currentBlockReward = 4;
    }
    circulatingSupply += currentBlockReward;
  }
