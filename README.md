...
[![Build Status](https://img.shields.io/travis/gohelpfund/insight-api/master.svg?branch=master)](https://travis-ci.org/gohelpfund/insight-api)
[![NPM version](https://img.shields.io/npm/v/@gohelpfund/insight-api.svg)](https://npmjs.org/package/@gohelpfund/insight-api)
...
> A Help blockchain REST and WebSocket API Service

This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/gohelpfund/insight-ui.
...
helpcore-node start  # to also start the service
```
...
- [Helpcore Node Help 4.x](https://github.com/gohelpfund/helpcore-node)

**Note:** You can use an existing Help data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` need to be enabled in `help.conf`, as well as a few other additional fields.

### Query Rate Limit

To protect the server, insight-api has a built-in query rate limiter. It can be configurable in `helpcore-node.json` with:
...
With all the configuration options available: https://github.com/gohelpfund/insight-api/blob/master/lib/ratelimiter.js#L10-17
...
- The response for the `/sync` endpoint does not include `startTs` and `endTs` as the sync is no longer relevant as indexes are built in helpd.
- The endpoint for `/peer` is no longer relevant connection to helpd is via ZMQ.
...
The `/status?q=getTxOutSetInfo` method has also been removed due to the query being very slow and locking helpd.
...
- (Medium)[How to setup a Help Instant-Send Transaction using Insight API - The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)

## Contributing

Feel free to dive in! [Open an issue](https://github.com/gohelpfund/insight-api/issues/new) or submit PRs.
